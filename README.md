...
초기에는 조금 더 큰 스케일의 게임을 만드려했다.

하지만, 프로젝트를 진행하면서 에셋이 부족하여 계속해서 기획이 변경되었다.

그때문에 기간이 조금 더 길어졌다.

프로젝트 중 가장 어려웠던 것은 기본적인 구조를 짜는 것이었다.

클래스가 최대한 재활용될 수 있게 설계하고 싶었고, 코드 중복이나 불필요한 참조또한 최대한 없애고 싶었다.

그러다 보니 상속구조나 참조가 계속해서 변경되어 혼란스럽기도 했다.

이러한 과정에서 확실히 배운것은 인터페이스에 대한 활용이다.

예를을어, Interaction에 대한 구체적인 동작은 아이템마다 다르지만 같은 키를 눌러 Interaction을 하게 구현해야 하는 상황이었다.

처음에는 Interaction을 하는 모든 객체는 Level에 올라가는 Actor를 상속받는다고 생각하여 InteractiveActor라는 클래스를 만들고 Interact라는 함수를 override하여 사용하려 했다.

그렇게 진행해도 동작은 했지만 불필요한 변수나 참조를 들고 있어야 했다.

이 부분을 없애기 위해 인터페이스를 정의하여 구현하게 만들었다.

이렇게 하니 다형성을 활용하면서도 불필요한 부분을 확실히 제거할 수 있었다.

이 과정을 분석하여 동일한 행위를 하는 관련없는 객체는 인터페이스를 통해 묶으면 된다는 결론을 내렸다.

그리고 GameInstance같은 Framework를 활용하는 방법을 배울 수 있었고 Character의 활용과 Anim을 적용하는 법을 배울 수 있었다.

GameInstance는 게임 진행중에 항상 유지되기 때문에 데이터를 갖고 있기에 적합한 클래스이다.

이 부분을 활용하여 DataTable을 GameInstance에 저장하여 필요할 때마다 해당 데이터를 받아오게끔 설계하였다.

이렇게 설계하니 클래스마다 Mesh나 String 등 갖고있어야 하는 부분을 통합하여 관리할 수 있어 편리했다.

한가지 아쉬운 부분은 엑셀을 통해 데이터를 입,출력할 수 있다고 알고 있는데 이부분을 경험해보지는 못했다.

또한, Character에 Anim을 적용하는 법에 대해 다시 학습하는 계기였다.

간단히 요약하자면, Character 클래스에 입력을 받은 후, 변수를 변경하고 이 변수를 AnimInstance에서 읽어와 애니메이션을 적용하는 방식이다.

이 과정에서 여러 연산을 통해 다양한 애니메이션을 섞을 수도 있다.

또한, Montage를 통해 하나로 합쳐진 애니메이션을 실행하게 할 수 있으며 이때, 노티파이를 통해 사운드를 재생하거나 Delegate처럼 활용할 수도 있었다.

마지막으로 게임을 만들면서 느낀 점은 Widget이 생각보다 많이 필요하다는 것이다.

여러 Widget을 만들면서 ListView를 활용하는 방법을 확실히 배웠다.

요약하자면 다음과 같다.
1. Data를 만들고 이를 ListView에 추가한다.
2. ListView의 EntryWidget은 IUserObjectListEntry를 구현해야 한다.
3. 인터페이스의 순수 가상 함수인 NativeOnListItemObjectSet 구현하여 Entry의 Data를 표시한다.

그리고 Landscape를 활용하여 Map을 구성해봤는데 여러 튜토리얼과 자료를 찾아보며 열심히 만들었다.

처음 해보는 과정이라 매우 어려웠지만 Layer를 쌓는다는 개념을 VFX를 제작할 때 이해하여 그나마 다행이었다.

이외에도 Niagara역시 경험해봤다.

튜토리얼을 따라한게 다이지만 재미있게 진행했던 과정이었다.


이번 프로젝트를 통해 다양한 경험을 하며 위에 언급한 내용에 대해 학습할 수 있었다.

한 가지 아쉬운 부분은 프로젝트를 거의 완성할 시기에 GAS(Game Ability System)에 대해 알게 되었고 이번 프로젝트에 적용했다면 좋았을 것 같다고 생각했다.

따라서, 이 프로젝트를 마치며 GAS에 대해 학습하고 GAS를 적용한 프로젝트를 하나 만들 계획이다.

그리고 프로젝트에 대한 회고(?)를 좀 더 열심히 써야겠다....
